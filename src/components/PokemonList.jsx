...
const PokemonList = () => {
    ...
    return (
        <div className="flex flex-col items-center gap-10 pt-3 relative w-full">
            ...
            <div id="pokemon-list" className={`grid gap-6 ${data.length > 1 ? "grid-cols-4" : ""}`}>
                <Cards data={data} />
            </div>
            <div className="flex gap-4 mb-10">
                <button className="border-2 border-black bg-amber-400 px-2 rounded-xl hover:bg-amber-500" onClick={() => setPage((prev) => prev > 0 ? prev - 1 : prev)} disabled={loading || page === 0}>
                    <i className="fa-solid fa-arrow-left"></i>  Prev 12
                </button>
                <button className="border-2 border-black bg-amber-400 px-2 rounded-xl hover:bg-amber-500" onClick={() => setPage((prev) => prev + 1)} disabled={loading}>
